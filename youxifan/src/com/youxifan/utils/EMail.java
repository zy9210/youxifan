...
public final class EMail implements Serializable
{
	 ...
	 */
	public EMail (String smtpHost, int smtpPort, boolean isSmtpTLS, 
	...
	public EMail ( String smtpHost, int smtpPort, boolean isSmtpTLS, 
			...
			String subject, String htmlMessage, boolean html)
	{
		setSmtpHost(smtpHost);
		...
		{
			if (html)
				setMessageHTML(subject, htmlMessage);
			else
				setMessageText (htmlMessage);
		}
	}	//	EMail
	 ...
	 * 	@return true if valid
	 */
	public boolean setTo (String toEMail, String toName)
	{
		...
		{
			if (toName == null)
				ia = new InternetAddress (toEMail, true);
			else
				ia = new InternetAddress (toEMail, toName);
		}
		catch (Exception e)
		{
			log.error(toEMail + ": " + e.toString());
			m_valid = false;
			return m_valid;
		}
		if (m_to == null)
...
//	private Properties			m_ctx;
	...
	/**	Logger							*/
	protected final transient Log log = LogFactory.getLog(EMail.class); 
...
//			}
	...
	public boolean setFrom(String newFromEMail, String newFromName)
	{
		...
		{
			log.warn(newFromEMail + ": " + e.toString()); 
			m_valid = false;
		}
		return m_valid;
	}   //  setFrom
	...
	public boolean isValid (boolean recheck)
	{
		...
		{
			log.warn("From is invalid=" + m_from);
			return false;
		}
		//	To
		...
		for (int i = 0; i < ias.length; i++)
		{
			...
			{
				log.warn("To(" + i + ") is invalid=" + ias[i]);
				return false;
			}
		}
		...
		{
			log.warn("SMTP Host is invalid" + m_smtpHost);
			return false;
		}
		...
		{
			log.warn("Subject is invalid=" + m_subject);
			return false;
		}
		return true;
	}   //  isValid
