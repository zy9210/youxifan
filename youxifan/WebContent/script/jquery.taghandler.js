...
(function ($) {
    ...
    $.fn.tagHandler = function (options) {
        if (typeof(options) == 'object' || typeof(options) == 'undefined') {
            ...
            return this.each(function () {
                ...
                if (opts.updateURL !== '') {
                    if (!opts.autoUpdate) { //非自动提交     手动点解保持图标   提交标签内容
                ...
                // initializes the tag lists    初始化 从url加载数据  initLoad 位  true
                ...
                if (opts.getURL !== '' && opts.initLoad) {
                    $.ajax({
                    	...
                    	//如果返回的data中没有availableTags 或 assignedTags 会出错   所以 加上data.availableTags判断
                            if (data.availableTags && data.availableTags.length) {
                            ...
                            if (data.assignedTags && data.assignedTags.length) {
                	...
                	//getURL不为空     不从服务器获取数据   只初始化assignedTags参数
                ...
                if (opts.allowEdit) {
                    ...
                    // the tagItem class that will remove the tag upon click  点解标签时删除 
    ...
    function addTag(tagField, value, tags, sort) {
		...
		submitTextField.val(tags.assignedTags.join(","));
        return tags;
    }
