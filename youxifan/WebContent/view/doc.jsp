...
<body class="question-page">
	...
	<div class="container">
		...
		<div id="content">   
			<div id="mainbar">    
				<div class="subheader">
					<h1  style="float:left;">${doc.title}</h1>
				</div> 
...
<c:forEach items="${doc.answers}" var="answer">                   
                    	...
                    	<div class="answer-details"  >        
							...
							<div class="started fr">
								<div class="user-info"> 
									<div class="user-gravatar32">
										<a href="/users/565319/richard">
											<div class=""><img src="${pageScope.contextPath}/${answer.creater.imageurl}" alt="" width="32" height="32"></div>
										</a>
									</div>
									<div class="user-details">
										<a href="/users/${answer.creater.userid}">${answer.creater.username}</a><br>
										<span class="reputation-score" title="reputation score" dir="ltr">296</span>
										<span title="1 silver badge">
											<span class="badge2"></span>
											<span class="badgecount">1</span>
                ...
                <div id="youranswer">
                	<form id="post-form" action="/questions/9890031/answer/submit" method="post" class="post-form">
                        ...
                        <h2 class="space">添加答案</h2>
                        <textarea id="wmd-input" class="wmd-input" name="post-text" cols="92"
        rows="15" tabindex="101"></textarea>
                        <div class="form-submit cbt">
                            <input id="submit-button" type="button" value="提交你的答案" onclick ="postanswer()" tabindex="110"> 
                        </div> 
					</form>
    			</div><!--youranswer -->
			</div>  <!--mainbar -->
...
function postanswer(){
	$.ajax( {
		type : "POST",
		url : "<%=request.getContextPath()%>/s/doc/${doc.docid}/answer",
		data : {"post-text":$("#wmd-input").val(),"doctype":"3"},
		dataType: "json",
		success : function(data) {
			if(data.success == "true"){
				alert("添加成功！");
				alert($("#wmd-input").html());
				$("#answers").append("<div>"+$("#wmd-input").val()+"</div>");
				$("#wmd-input").val("");
			}
			else{
				alert(data.msg);
